...
export const getSsrClient = (
  ...
  const customFetch: typeof fetch = async (input, init) => {
    ...
    setCookieHeaders.forEach((cookieHeader) => {
      // Parse the Set-Cookie header (name, value, and all attributes) using
      // the cookie library, which handles decoding and attribute normalization.
      const { name, value, ...options } = cookie.parseSetCookie(cookieHeader)
      astroContext.cookies.set(
        name,
        value ?? '',
